...
const XionWallet = () => {
  ...
  const handleAuth = useCallback(async () => {
    ...
    } finally { authInProgress.current = false; }
  }, [bech32Address, walletAuth, dispatch, toast]);

  const handleConnect = useCallback(() => { setShow(true); }, [setShow]);
  ...
  useEffect(() => {
    if (isConnected && !isConnecting && bech32Address && !isAuthenticated && !authInProgress.current) handleAuth();
    if (!isConnected && isAuthenticated && !isConnecting) handleDisconnect();
  }, [isConnected, isConnecting, bech32Address, isAuthenticated, handleAuth, handleDisconnect]);

  if (isConnecting || isLoading) return <Loading text="Loading wallet..." />;
